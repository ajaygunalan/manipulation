...
# Get the path to the ur5e.urdf file in the parent directory
current_dir = os.path.dirname(os.path.abspath(__file__))
parent_dir = os.path.dirname(current_dir)
urdf_path = os.path.join(parent_dir, "ur5e.urdf")

# Print the constructed URDF path
print(f"Loading URDF from: {urdf_path}")

# Create the visualizer
visualizer = ModelVisualizer(meshcat=meshcat)

# Add the UR5e model using the file path
visualizer.AddModels(filename=urdf_path)
# visualizer.AddModels(url="package://drake_models/franka_description/urdf/panda_arm_hand.urdf")
# 
